...
std::enable_if_t<is_serializer_v<Interface>> serialize(Interface& s, const std::unordered_set<Parameters...>& v, Arguments&& ... args) {
    detail::serialize_sequence(s, v, std::forward<Arguments>(args)...);
}

template <typename Interface, typename ... Parameters, typename ... Arguments>
std::enable_if_t<is_deserializer_v<Interface>> deserialize(Interface& d, std::unordered_set<Parameters...>& v, Arguments&& ... args) {
